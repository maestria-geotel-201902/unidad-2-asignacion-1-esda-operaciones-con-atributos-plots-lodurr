...
## Reading layer `PROVCenso2010' from data source `/home/ingdi/unidad-2-asignacion-1-esda-operaciones-con-atributos-plots-lodurr/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
proven17  %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿El costo de la vida?')) %>%
  plot(breaks = 'jenks')
```
...
#Rótulos: nombres de las provincias
proven17 %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿El costo de la vida?'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
...
``` r
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('Principales problemas de su barrio o comunidad: ¿El costo de la vida?'))
##                 TOPONIMIA
## 1       DISTRITO NACIONAL
## 2                    AZUA
## 3                 BAORUCO
...
proven17 %>%
  ...
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'Principales problemas de su barrio o comunidad: ¿El costo de la vida?: Si') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
proven17 %>%
  ...
  ggplot() + aes(x = TOPONIMIA, y = valor, fill = variable, group = variable) +
  geom_col(position = 'dodge') + scale_y_continuous() +
  theme(axis.text.x = element_text(angle = 90), text = element_text(size = 7))  + coord_flip()
```
...
``` r
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0603') %>% 
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿El costo de la vida?'))
##   Principales problemas de su barrio o comunidad: ¿El costo de la vida?: Si
## 1                                                                        55
##   Principales problemas de su barrio o comunidad: ¿El costo de la vida?: No
